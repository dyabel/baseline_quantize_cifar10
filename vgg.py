...
#            utils.QW()
 ...
 #           utils.QW()
        ...
        for m in self.modules():
            #if isinstance(m, utils.MyConv):
                #print("find utils.Conv2d")
            if isinstance(m, nn.Conv2d):
            #if isinstance(m, utils.Conv2d):
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            #conv2d = utils.MyConv(in_channels, v, kernel_size=3, padding=1)
            #relu = utils.ReLU(inplace=True)
            ...
            if batch_norm:
                ...
                layers += [conv2d, relu]
            in_channels = v
    return nn.Sequential(*layers)  # 加*表示传递元组(多个参数)
